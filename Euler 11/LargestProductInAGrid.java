...
public class LargestProductInAGrid {
   ...
   private static long largestProduct(int[][] array){
       var product = 0;
       var largest = 0;

       //horizontal right
       for(var row = 0; row < array.length; row++){
           for(var col = 0; col < array[row].length - 3; col++){
               product = array[row][col] * array[row][col + 1] *array[row][col + 2] * array[row][col + 3];
               largest = Math.max(largest, product);
           }
       }
      
       // < Horizontal left and vertical left aren't needed for this solution > 
       // < Horizontal left is provided for learning purpose only >

       //horizontal left 
       /*
       for(var row = 0; row < array.length; row++){
           for(var col = array.length; col > 3; col--){
               product = array[row][col - 1] * array[row][col - 2] *array[row][col - 3] * array[row][col - 4];
               largest = Math.max(largest, product);
           }
       }
       */
       ...
       //diagonal left
       for(var row = 0; row < array.length - 3; row++){
           for(var col = 3; col < array[row].length; col++){
               product = array[row][col] * array[row + 1][col - 1] *array[row + 2][col - 2] * array[row + 3][col - 3];
               largest = Math.max(largest, product);
           }
       }
   ...
   public static void  main(String... args){
       var array = data();
       //printArray(array); // < use if you want to see grid >
